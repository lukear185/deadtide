...
Claude Code 自身が作って Claude Code 自身が使う道具の洗い出し。対象は **DEADTIDE(単一HTML・canvas 2D)** と **Yadocho(UE5)** の2つ。⚠着手はまだ。

---

## いま既にあるもの

重複提案を防ぐための棚卸し。詳細は `NOTES_道具.md`。
...
| `test_shot.js` | 実画面スクショ。オプション50以上(`st2` `dev` `arena=` `fxdemo` 等) | Claude→人 |
...
| `?dev=1` `?sfx=1` `?edit=1` | DEVモード／音の確認／マス編集モード | 人 |
...
2. 絵の前回比較(視覚回帰)が無い
3. `test_layout.js` と `test_balance.js` は必ず成功扱い＝自動判定できない
4. `test_sfx.js` が公式手順に載っておらず流し忘れる
5. DEV系画面(マス編集・図鑑・音の確認・検証場)に自動検査がほぼ無い
...
7. `test_shot.js` に `--help` が無い
8. デバッグモードの入口が URL 直打ちのみ
...
### 5. つまみ抽出 + 上書き口 + 書き戻し器(`tool_tune.js`)
...
> 次点(効き目大だが手間や前提が重い): **セーブ移行の門番**(版番号が無いのは実在の穴)／**道具のSkill化＋hooks**／**未実行コード地図**。
...
|---|---|---|---|---|
| ⭐見取り図シート | 全SFXをスペクトログラム＋波形で1枚に束ねる | ffmpeg `showspectrumpic` / `showwavespic` / `tile`(CLIとして呼ぶだけ＝伝播なし) | 小 | 大 |
| ⭐帯域の門番 | NOTES_音の掟を数値検査にして FAIL | ffmpeg `aspectralstats` `astats`／Node側FFTなら `fft.js`(MIT) | 中 | 大 |
| ⭐素材の在庫台帳 | zipを開かず中身を索引化・UCS対訳で検索 | `yauzl`/`adm-zip`(MIT)＋UCS(パブリックドメイン)。素材=Sonniss(商用可)・Kenney Audio(CC0)・Freesound CC0・ZapSplat CC0のみ | 小 | 大 |
...
| 道具 | 何をする | 実現方法とライセンス | 手間 | 効き目 |
|---|---|---|---|---|
| ⭐コマ撮りシート(`test_seq.js`) | Nフレームを等間隔で撮って1枚のタイルに | ヘッドレスChrome＋固定dtステップ関数＋ffmpeg `tile` | 中 | 大 |
| 粒エディタ(`fx_edit.html`) | 初速・重力・寿命・色カーブをスライダで即プレビュー、JSONで保存 | 素のcanvas 2D＋JSON。埋め込みは `tool_part.js` と同じ枠置換。JSONスキーマの決め方だけ tsParticles(MIT) を参考に | 中 | 大 |
| VFXカタログ＋画像回帰 | 全エフェクトを格子で同時再生、前回と自動比較 | `?dev=1` に相乗り＋`pixelmatch`(ISC・依存ゼロ・1ファイル同梱可) | 小 | 大 |
| 手応え計測(`test_feel.js`) | ヒットストップms・画面揺れ振幅と減衰・フラッシュ持続を数値化 | DEV時のみの計測フック＋ヘッドレス実走。相場=3〜12フレーム/40〜80ms、揺れ2〜3px(弱)〜8〜10px(強)、指数減衰 | 中 | 大 |
| 粒テクスチャの手続き生成(`tool_tex.js`) | Kenneyに無い形(リング・線状火花・十字閃光)を生成 | `pngjs`(MIT)＋`simplex-noise`(MIT)。`tool_part.js` の48px/rgba/base64経路に合流 | 中 | 中 |
| 光の層(加算＋安いブルーム)と撮り比べ | 光る物だけ低解像度canvasに描いて加算合成、URLフラグでON/OFF | `globalCompositeOperation='lighter'`＋縮小→拡大の擬似ブラー | 中 | 大 |
| 粒の予算番(`perf_fx.js`) | 60fpsを割る粒数を割り出して上限に | CDP `Emulation.setCPUThrottlingRate`＋`performance.now()`。相場は drawImage 1000〜2500スプライト/60fps | 小 | 中 |
...
| 道具 | 何をする | 実現方法とライセンス | 手間 | 効き目 |
|---|---|---|---|---|
| ⭐コンタクトシート撮り | 全種を実表示サイズのまま1枚に並べて撮る | ヘッドレスChrome＋描画関数だけを呼ぶ一時HTML。依存ゼロ | 小 | 大 |
...
| パレット検査 | コントラスト比・色覚多様性・色数を機械判定 | WCAG2の比は仕様公開の四則演算＝自前実装が最安。`culori`(MIT)／`chroma.js`(BSD-3・0.4以前はGPLなので拾わない)／`@bjornlu/colorblind`(MIT) | 小 | 大 |
| シルエット判別テスト | 白黒化→縮小ぼかしで「見分けがつかない組」を一覧に | 追加ライブラリ不要。輝度化→1/4縮小→全ペアIoU＋平均輝度差 | 中 | 大 |
| UIレイアウトの実測検査 | 手打ち矩形をやめ、実測で重なり/はみ出し/タップ領域を判定 | `getBoundingClientRect()`＋canvas側に `__uiRects()` を足す。下限は WCAG 2.2 の 24×24 CSS px、推奨は 44×44pt | 中 | 中 |
...
| 道具 | 何をする | 実現方法とライセンス | 手間 | 効き目 |
|---|---|---|---|---|
| Chrome直結ドライバ(`tool_cdp.js`) | `--dump-dom`＋正規表現をやめ、実ブラウザと双方向で会話する土台 | CDP。Node v24.18 は `WebSocket`/`fetch` 標準搭載＝**npm依存ゼロ**で書ける | 中 | 大(土台) |
...
| 不変条件の総当たり | 「シェア合計100」「資源は負にならない」「保存→再開で一致」をランダム操作列で破る | `fast-check`(MIT・model-based testing＋shrinking)。依存を増やさないなら `chkShares` を一般化した自前簡易版で8割 | 中 | 中 |
...
| 道具 | 何をする | 実現方法とライセンス | 手間 | 効き目 |
|---|---|---|---|---|
| ⭐つまみ抽出＆上書き口 | 数値を機械可読に吸い出し、外から差し替える口を作る | `/*@tune*/` 目印＋`tune.schema.json`＋`?tune=`/localStorage。依存ゼロ | 中 | 大 |
| ⭐値の書き戻し器 | 詰めた JSON を index.html に反映して輪を閉じる | 錨の一意性を assert してから置換 → そのまま検査まで1コマンド | 小 | 大 |
...
| 道具 | 何をする | 実現方法とライセンス | 手間 | 効き目 |
|---|---|---|---|---|
| ⭐出す前の関門(リリースゲート) | 検査を決まった順で全部流し、1本でも落ちたら止める | `spawnSync`＋`.git/hooks/pre-push`。依存ゼロ。**[UE5]** は `RunUAT.bat BuildCookRun`＋`-run=DataValidation` を並べる | 小 | 大 |
| セーブ移行の門番 | セーブに版番号を持たせ、v1→v2→v3 の移行関数を並べて旧セーブ標本を毎回全通し | `MIG=[…]` 配列＋`save_fixtures/*.json`(golden fixture)。形の検査を固くするなら `zod`(MIT)／`valibot`(MIT)は**ツール側だけ**。**[UE5]** は `FCustomVersionRegistration`＋`UObject::Serialize` | 中 | 大 |
| 実機セーブの持ち帰り | 実機の localStorage を1本の文字列で書き出す/読み戻す | `CompressionStream('deflate')`(ブラウザ標準)→base64。`?dev=1` 限定にする | 小 | 大 |
...
| 数値表の自動生成＋設計書との照合 | 実装の数値から Markdown 表を作り、`DESIGN.md`/`NOTES_数値.md` のズレを報告 | `test_balance.js` の取り出し部を再利用。`<!-- AUTO:units -->`〜`<!-- /AUTO -->` を差し替え(ICONS-START/END と同じ流儀) | 小 | 大 |
...
| 道具 | 何をする | いつ使う |
|---|---|---|
| `?perf=1` | **実機**で「塗り○倍/グラデ○枚/内訳」を画面下に出す | **熱いと言われた時** |
| `node tool_paint.js` | 場面ごとの塗り倍率を重い順に(前後比較) | 直した後に減ったか見る |
| `node tool_paint.js --check` | 上限超えで落ちる(`tool_release` の7本目) | 毎回(自動) |

⚠⚠**`tool_bench.js`(時間)と役割が違う**=「重い」は時間、「熱い」は塗った面積。
**(173)の「塔1台につき灯り1枚」は時間には出ず、塗りの物差しにだけ出た**。
📌詳しい当て方と、作る時に踏んだ穴3つは [[NOTES_道具]]。
