...
- `node test_frontline_browser.js`：Chromeのある環境で試作を実際に起動し、タップ配置・開始・停止・画面寸法を確認。横持ち相当とPC寸法の画像を保存する。
- `node test_frontline.js`：開放戦場の実座標戦闘、遮蔽・迂回、配置費用・上限、専用URLの起動・保存分離・操作配線を検査する。DOMはスタブなので実ブラウザの表示確認とは別。
...
| 🎵`tool_bgm.js` | **BGMファイルの埋め込み**=`bgm_src/`(gitignore)のmp3を低ビットレートに焼いて index.html の BGM-DATA へ。曲の一覧と出所URLはこのファイルの `LIST` が正 | Claude |
