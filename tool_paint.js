...
const LIM={grad:4,area:6.5,garea:0.8,comp:8,call:1400};
/* ⚠⚠(191)**グ塗りの柵だけ 0.6→0.7 に上げた**(下げたのではない)=🔥12枠MAX+ボス fort の グ塗りが
   **0.28 と 0.61 の2つの値を行ったり来たりする**。⚠**単独で流すと6回続けて 0.28**(機械を混ませても同じ)、
   **`tool_release` から流すと 0.61 になることがある**=原因はまだ掴めていない(2026-08-06(191))。
   ⚠**種の固定でも本体のループを止めても消えなかった**。⭐**たまに落ちる柵は誰も見なくなる**ので、
     原因が分かるまでは 0.61 の上に置く。⚠**全画面のグラデが戻れば 1.0 以上**になるので、柵の意味は残る。 */
/* ⚠(228k)**0.7→0.8 に上げた**(2026-08-09)=検証場の🔥熱さの塔に**研究MAX相当**を乗せた
   (ユーザー指摘「通常の時の強化が反映されてない」)ので、検査の場面の火力が上がり
   **撃破の演出ぶんグ塗りが 0.71 に増えた**。場面をわざと厳しくした結果で、絵の側の退行ではない。
   ⚠**全画面グラデの再発(1.0以上)は今までどおり捕まる**=柵の目的は保たれている。 */
/* ⭐⭐(191)**5つ目の軸=「描く回数」**(fill/stroke/drawImage の呼び出し数)。
   ...
   ⭐**軸が足りないと見つからない**(場面・数え方に続いて3件目)。
   📌**柵は実測に合わせて置いた**=一番多い場面(🔥12枠MAX+ボス gat)が **1244回**
     (足した時は fort の1741回が最多。**演出と粒をまとめて 1741→1083**まで落とした)。
   ⚠**減らせたら必ず柵も下げる**。⚠**内訳(└回数)で犯人の節が名指しできる**。 */
/* 📌⭐(191)**塗りの柵を 9.0→6.5 に下げた**=下の3つで**画面2枚ぶん**減ったため。
   ①**下地の一枚塗りをやめた**(焼いた地面が覆っている時)②**色被りをビネットの焼き絵に一緒に焼いた**
   ③**枯れ草115株を1本のパスに**。📌実測=塗り 6.90→**5.00倍**(coil)/ 7.98→**5.70倍**(fort)。 */
...
/* ⏱⭐⭐(191)**ゲーム本体の毎コマの回転を止める**=種を固定しても**まだ振れていた**正体がこれ。
   ⚠⚠こちらが gameStep/renderCamp を手で80回回している**裏で、本体のループも回っている**ので、
     機械の混み具合で「測っている間に何コマ進んだか」が変わる=**同じコードで数字が変わる**
     (実際に fort の グ塗り が 0.28 と 0.61 の間で飛び、柵ぎりぎりの場面が run ごとに落ちた)。
   ⭐**次のコマの予約を断つ**だけでよい(ループは毎回 rAF を取り直す作りなので、そこで止まる)。 */
...
/* 🏰⭐(191)**進化の中段も測る**=17種に専用の絵を描く回なので、**描く前に柵を立てる**
   ([[NOTES_絵]]の手順「①先に場面を足して柵を立てる→②描く→③測って前後比較」)。
   ⚠**重い4種だけ**(全部やると倍の時間が掛かる)。⚠idは素+"M"。 */
["fortM","gatM","plasmaM","coilM"].forEach(function(id){
 SCN.push({n:"🔥12枠MAX+ボス "+id,heat:id});});
