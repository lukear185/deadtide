...
function checkGacha(){
 ...
 if(HEROES.length!==28){console.log('FAIL: 英雄が28種でない '+HEROES.length);process.exit(1);}/* 🐉(252)炎龍ウグニウス */
 ...
 const want={1:5,2:4,3:4,4:4,5:11};/* 💥(256)爆拳のレンで★5は11人 *//* 💧(254)クループで★5は10人 *//* 🌌(254)ディメンションナイトで★5は9人 *//* 🐉(252)炎龍ウグニウスで★5は8人 */
 ...
 {const ids=Object.keys(H_CMB);
  ...
  for(const id of ids){
   ...
   for(const r of A){
    ...
    /* ⚔(250)**居合(hZan)も広く取る**=横一文字は「角度を変えずに前へ走らせる」で作るので、
       ずれxが大きいのが正しい姿(騎槍の突き出しと同じ扱い)。⚠それ以外は今までどおり14。 */
    const LIM=(id==='hDark')?30:(id==='hZan'?28:14);
...
function checkBusGim(){
 ...
 const want=['','','wire','pop','gas','','','','',''],got=[];/* 🧹(264)⑥〜⑩は掃討便=道の仕掛けは無い */
 for(let si=0;si<STAGES.length;si++){
...
/* 🚌🧹⭐⭐⭐(264)**掃討便**(面⑥〜⑩)。⚠見るのは7つ:
   ①走る便(面①)は掃討便にならない ②面⑥は掃討便になり、広場の真ん中から始まる
   ③群れが盤に居る(薄いと掃討にならない) ④轢けば倒れて⚡スコアと連なりに乗る
   ⑤外周の外へ出られない ⑥時間で終わる(⚠負けは無い) ⑦装備(ノコ)が当たりを広げている */
function checkSwp(){
 const F=m=>{console.log('FAIL: '+m);process.exit(1);};
 META.sc=STAGES.map(()=>D5.map(()=>1));META.sclr=STAGES.map(()=>1);
 META.bres=[0,0,0];META.bup={};META.beq={};
 /* ① 走る便 */
 META.stg=0;setDiff=BNS_D;startSolo();bnsPreSkip();
 if(G.swp)F('面①が掃討便になっている');
 /* ② 面⑥ */
 META.stg=LAP_N;setDiff=BNS_D;startSolo();bnsPreSkip();
 const S=G.swp,C=G.players[0],B=C.bus;
 if(!S)F('面⑥が掃討便になっていない');
 if(Math.abs(B.x-SWP_W/2)>1||Math.abs(B.y-SWP_W/2)>1)F('広場の真ん中から始まっていない');
 /* ③ 群れの厚み */
 let live=0;for(const z of S.en)if(!z.dead)live++;
 if(live<SWP_KEEP*.9)F('群れが薄い '+live+'/'+SWP_KEEP);
 /* ⑦ ノコを付けると当たりが広がる(=装備がこの面で効いている) */
 {const o0=Math.max(BUSH_OUT*(B.rX||1),(B.blOut||0)*(B.rX||1),(B.s3Out||0)*(B.rX||1));
  bupApply(B,{},{blade:1,saw2:1});
  const o1=Math.max(BUSH_OUT*(B.rX||1),(B.blOut||0)*(B.rX||1),(B.s3Out||0)*(B.rX||1));
  if(!(o1>o0))F('ノコを付けても当たりが広がらない '+Math.round(o0)+'→'+Math.round(o1));
  bupApply(B,{},{});}
 /* ④ 轢けば倒れて⚡と連なりに乗る(⚠バスの鼻先へ置く) */
 {const k0=B.kill|0,s0=B.score||0;
  B.ang=0;B.vx=0;B.vy=0;
  for(let n=0;n<8;n++){const z=S.en[n];if(!z)break;
   z.x=B.x+40+n*6;z.y=B.y+(n-4)*10;z.hp=1;z.mhp=1;z.stn=0;}
  swpCrush(S,C,B,.05);
